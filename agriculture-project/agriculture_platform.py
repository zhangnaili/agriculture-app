...
def login_page():
    ...
    with tab2:
        with st.form("register_form"):
            ...
            user_type = st.selectbox("用户类型", ["普通用户", "农场主", "农业专家", "管理员"])
            ...
            if user_type == "农场主":
                ...
                farm_info['location'] = st.text_input("所在地")
                farm_info['main_crops'] = st.text_input("主要作物")
            ...
            if register_button:
                if not new_username or not new_password:
                    ...
                    success, message = register_user(new_username, new_password, user_type, farm_info)
                    if success:
                        st.success(message)
                    else:
                        st.error(message)
...
def main():
    ...
    if user_type == "管理员":
        menu_items = ["数据驾驶舱", "智能规划器", "风险模拟器", "效益分析", "聊天咨询", "数据管理", "个人中心",
                      "系统管理",
                      "关于项目"]
